...
def as_dict(x: Any) -> Dict:
    """Coerce a case entry into a dict.

    Some models return a single-element list wrapping the result (or even
    a multi-element list). We always take the first dict if available so
    downstream metric/report code can rely on .get().
    """
    if isinstance(x, list):
        x = x[0] if x and isinstance(x[0], dict) else {}
...
def detect_model_type(model_id: str) -> str:
    ...
    # Not a LoRA — read the model's own config
    try:
    ...
    if model_type == 'mistral3':
        return 'mistral3_vlm' if 'bf16' in model_id.lower() else 'mistral3_vlm_fp8'
...
def load_model_and_tokenizer(model_id: str):
    ...
    is_lora = False
    adapter_cfg_local_path = None

    if os.path.isdir(model_id) and os.path.exists(os.path.join(model_id, "adapter_config.json")):
        is_lora = True
        adapter_cfg_local_path = os.path.join(model_id, "adapter_config.json")
    else:
        try:
            adapter_cfg_local_path = hf_hub_download(repo_id=model_id, filename="adapter_config.json")
            is_lora = True
        except Exception:
            is_lora = False
    ...
    if mtype.startswith('mistral3_vlm'):
        ...
        if is_lora:
            ...
            # Read base model path from adapter config (works for local OR Hub)
            with open(adapter_cfg_local_path, 'r') as f:
...
def calculate_field_metrics(output_cases, gold_cases):
    ...
    for f in fields:
        correct = sum(
            1 for o, g in zip(output_cases, gold_cases)
            if as_dict(o).get(f) == g.get(f)
        )
...
def calculate_overall_accuracy(output_cases, gold_cases):
    exact = sum(
        all(as_dict(o).get(k) == g.get(k) for k in g.keys() if k not in IGNORE_KEYS)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Skip inference and regenerate the report from an existing output JSON file.",
    )
    ...
    # ------------------------------------------------------------------
    # Report-only mode: skip model loading & inference, just regenerate
    # ------------------------------------------------------------------
    if args.report_only:
        print(f"Report-only mode: regenerating report from {args.report_only}")
        generate_detailed_report(model_id, args.report_only, gold_file, rep_path, perf={})
    ...
    for idx, case in enumerate(cases, start=1):
        ...
        # Always coerce to a single dict per case for downstream metrics
